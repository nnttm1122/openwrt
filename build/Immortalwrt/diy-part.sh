...
# 后台IP设置
export Ipv4_ipaddr="192.168.10.11"            # 修改openwrt后台地址(填0为关闭)
...
export Rootfs_partition_size="980"            # 系统分区大小,每个机型默认值不一样 (填写您想要的数值,默认一般300左右,数值以MB计算，填0为不作修改),如果你不懂就填0
...
export DNS_Settings="202.103.224.68"                     # 旁路由设置 DNS(填入DNS，多个DNS要用空格分开)(填0为不作修改)
...
export Customized_Information="Nnttm $(TZ=UTC-8 date "+%Y.%m.%d")"  # 个性签名,你想写啥就写啥，(填0为不作修改)
